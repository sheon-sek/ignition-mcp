...
Status: **COMPLETE** — G2 closed on 2026-09-20.
...
5. **`GATEWAY_MODULES_ENABLED` is a whitelist, not a request to add one module.** The first G2 harness enabled only `com.inductiveautomation.mcp`, unintentionally suppressing Historian Core, Alarm Notification, and JDBC driver modules. `ACCEPT_MODULE_CERTS` remains limited to the third-party MCP module. The G2 fixture now declares the exact minimal built-in module set it needs and gates provisioning on the required authenticated `/openapi.json` routes; `modules/healthy` is diagnostic state, not sufficient proof that a capability route is registered.
6. **Named Query `sqlType` is Designer's own enum, not `java.sql.Types`.** The hand-authored fixture declared pagination parameters with `sqlType: 4` believing it was INTEGER; Designer reads 4 as Float4, coerced the handler's integers to `2.0`/`0.0`, and MariaDB rejected `LIMIT 2.0 OFFSET 0.0` (`SQLSyntaxErrorException` localized only from the Gateway-side log captured in the artifact). Designer's enum: 0=Int1, 1=Int2, **2=Int4**, 3=Int8, 4=Float4, 5=Float8, 6=Boolean, 7=String, 8=DateTime, 20=ByteArray.
7. **Diagnostics must be bounded.** An unbounded `docker exec cat` in the always-run diagnostics step hung both matrix jobs until cancellation (the container's `wrapper.log` is not a plain readable file). Every container-filesystem diagnostic call gets `timeout` plus detached stdin, and log copies must prune symlinks before `actions/upload-artifact` stats them. The Gateway's structured internal `system_logs.idb` — plain-text searchable via `strings` — is the reliable exception-cause channel.
...
6. **G2 harness (final):** `tests/harness/phase2-live/` provisions an exact-patch Gateway plus a disposable MariaDB container, a test-only Named Query project, historical Tag/UDT fixtures, and the CI-only `MCP_CI_AUDIT` profile through Native REST. The datasource driver/translator are discovered from the live Gateway's installed database resources: the first G2 attempt failed provisioning with `422 Invalid reference: 'PostgreSQL'`, proving fixture assumptions must be validated against the target. Subsequent runs exposed credential wire-shape, Historian path, workflow-registration, module-whitelist, Jython enum-normalization, and Named Query `sqlType`-enum defects (see the live-iteration lessons). `probe.py` performs exact 13-Tool Runtime discovery, per-Tool real smoke calls with observable domain assertions (config-tree `dataType: "Int4"`, exact `tag_query` fixture path set, D10 `nextOffset: 2` continuation, D28 null continuation marker), canonical-error checks, D27/D28-aware binding classification, and exact external capability verification; `.github/workflows/phase2-live-g2.yml` ran the 8.3.8 required + 8.3.9 compatibility matrix from the open Phase 2 PR. `alarm_pipeline_status` smoke adapts between a live pipeline and the canonical `not_found` negative without fabricating success.

**G2 is closed.** The [required 8.3.8 row](../compatibility/evidence/g2-8.3.8-mcp-2026021307/README.md) is `VERIFIED` with native binding `VERIFIED_WITH_LIMITATION` under the exact D27 tuple and `d27ExceptionApplied: true`; the [8.3.9 candidate row](../compatibility/evidence/g2-8.3.9-mcp-2026021307/README.md) ran the identical full smoke on the byte-identical deterministic Runtime ZIP and records the honest fail-closed `UNVERIFIED_LIMITATION` / `FAILED_NATIVE_BINDING` with `d27ExceptionApplied: false` — the D27 exception does not propagate. Final implementation commit [`852fc37c3c116db7addb924d4a9962f885c5e290`](https://github.com/sheon-sek/ignition-mcp/commit/852fc37c3c116db7addb924d4a9962f885c5e290): [G2 run 35534976018](https://github.com/sheon-sek/ignition-mcp/actions/runs/35534976018), same-SHA [CI](https://github.com/sheon-sek/ignition-mcp/actions/runs/35534976025) and [G0](https://github.com/sheon-sek/ignition-mcp/actions/runs/35534976030) passed. Runtime ZIP SHA-256 `5fa7f06a6bde2cc3ccc876e250aac569fe710aa2688349192e903903a3b6cde2`. D21 deployment compatibility remains `UNTESTED` on both rows; L3/L4 acceptance is the committed raw protocol evidence plus schema-validated live results.
