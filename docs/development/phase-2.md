...
6. **G2 harness (live iterations in progress):** `tests/harness/phase2-live/` provisions an exact-patch Gateway plus a disposable MariaDB container, a test-only Named Query project, historical Tag/UDT fixtures, and the CI-only `MCP_CI_AUDIT` profile through Native REST. The datasource driver/translator are discovered from the live Gateway's installed database resources: the first G2 attempt failed provisioning with `422 Invalid reference: 'PostgreSQL'`, proving fixture assumptions must be validated against the target. Subsequent runs exposed credential wire-shape, Historian path, workflow-registration, and module-whitelist defects. `probe.py` performs exact 13-Tool Runtime discovery, per-Tool real smoke calls, canonical-error checks, D27/D28-aware binding classification, and exact external capability verification; `.github/workflows/phase2-live-g2.yml` runs the 8.3.8 required + 8.3.9 compatibility matrix from the open Phase 2 PR and manual dispatch. After G2 closes it should be reduced to manual `workflow_dispatch` like the historical G1 gate. `alarm_pipeline_status` smoke adapts between a live pipeline and the canonical `not_found` negative without fabricating success.

**G2 has real failure evidence but no passing row yet.** Run `35524729067` reached authenticated REST and module setup on both 8.3.8/8.3.9, then failed before the dual-plane probe because the harness had whitelisted only the MCP module and the Historian provider REST route was absent. The harness now uses the exact required module whitelist plus OpenAPI route readiness. L3/L4 acceptance remains NOT RUN until a subsequent workflow reaches `probe.py`; Phase 2 stays open.
