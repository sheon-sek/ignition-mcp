...
2. Compare with the Gateway. The same build already installed is `OK` and uploads nothing. A Module
   the listing reports in any state but `ACTIVE` is refused before any write, because a Module the
   Gateway does not run hosts no route. A lower build is always refused. A newer build needs the
   upgrade acceptance, which `--yes` covers.
...
4. Install the Module. This needs `--accept-certificate` and `--accept-eula`; a missing acceptance
   ...
   skipped.
5. Restart the Gateway and wait until the Module runs with the new build. A Module that comes back in
   another state fails the step, which names the state and what the listing reports beside it, such
   as `onStartup disabled`.
