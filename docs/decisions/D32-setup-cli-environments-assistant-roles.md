...
**Amendment, 2026-09-25 (closes issue #79).** The `ignition-mcp-rest` token no longer copies the Security Level of the setup key. It holds one dedicated level, `Authenticated/IgnitionMcpRest`, and nothing else:

- `setup` creates the level with D20's procedure, the one the role levels use. In `prod` it needs `--provision-security-levels`, as section 5 decides for every Security Level.
- `setup` adds the level to the General Settings entries Native REST checks for an API token. On Ignition 8.3.8, `readPermissions` (Gateway Read) opens every read route the REST server calls and `writePermissions` (Gateway Write) opens every write route, while `accessPermissions` (Gateway Access) opens nothing for a token. `prod` grants `readPermissions`. `dev` grants `readPermissions` and `writePermissions`. `accessPermissions`, `designerPermissions` and every other setting are never changed.
- The General Settings edit follows D20: read the `security-properties` singleton with its signature right before the write, change only the entries that need the level, write with the signature as the precondition, and read it back. The read-back must equal the written settings exactly. An `AllOf` entry that lists levels is refused before any write, because an added level would lock out everyone who holds only the others. An entry that lists no level already lets every token through and is left alone.
- The level and each entry the level was added to are recorded as created. `reset` removes exactly those, entries first and the level last, with the same procedure. It never empties an entry, because an empty entry lets every token through. A move from `dev` to `prod` removes the recorded write entry and reports `CHANGED`. A move back adds it again.
- A token that still holds the setup key's level, as Phase 7 created it, moves to the dedicated level on the next `setup` run without the acceptance for overwriting a hand edit. Its step reports `CHANGED` with the old and the new level. Any other difference in the token keeps the section 10 rule.
