...
| `setup` fails on the Module with `module_not_active`. | The Gateway has the Module but is not running it, so it hosts no route and the Server Config step would fail with a 404. The reason names the state and the rest of the listing, such as `onStartup disabled`. A Module the Gateway is not allowed to start stays `INACTIVE` with `onStartup disabled`: check `GATEWAY_MODULES_ENABLED` in the Gateway's environment. |
