...
- `docs/decisions/D01–D32` plus `INDEX.md` are **binding**. Read the relevant decision before changing behavior. Changing a decided rule needs an explicit new decision or amendment section. Never make silent edits.
- Delivery is phase-gated (D26). Phases 0–3 (G0/G1/G2/G3) are closed and frozen; see `docs/development/phase-{0,1,2,3}.md`. Phase 4 (G4) is closed and merged; see `docs/development/phase-4.md`. Its deferred items are tracked in issues #39 and #41. Phase 5 (G5) is closed and merged; see `docs/development/phase-5.md`. Its deferred item is tracked in issue #51. Phase 6 (G6) is closed and merged; see `docs/development/phase-6.md` for the G6 checklist and the evidence rows behind each item, and `docs/operations/runbook.md` for the operator procedure. #39, #41 and #51 stay open as deferred work, and #57 holds the items the Phase 6 ruling moved out of v1. Phase 7 (G7, the setup CLI decided in D32) is in progress on `phase-7/setup-ux`; see `docs/development/phase-7.md`. **A next phase starts only on a new user-directed branch.** Don't begin next-phase work on `main` without being told to.
