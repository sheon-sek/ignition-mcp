def onToolCalled(builder, items):
	...
	logger = system.util.getLogger("IgnitionMCP.Runtime.TagCreate")
	...
	AUDIT_ACTION = "ignition-mcp.tag_create"
	...
	# as input - D30 2 gives a create no Precondition token - and publishes the
	# created node's own one as Observed state so the caller's next tag_update or
	# tag_delete has a token. The rule and its golden vectors are
	...
	# The handler fixes the Gateway's collision policy: Abort is what makes this a
	# create rather than a merge, and D11 forbids overwriting - an existing target is
	# conflict in Preflight, and a target that appears in the race window aborts
	...
	HARD_MAX_ITEMS = 100
	POLICY_MAX_ITEMS_FIELD = "tagCreateMaxItems"
	PATH_MAX_BYTES = 2048
	...
	def policyProblem(document):
		...
		if hasKey(document, POLICY_MAX_ITEMS_FIELD):
			...
			if isinstance(maxItems, bool) or not isinstance(maxItems, (int, long)) or maxItems < 1 or maxItems > HARD_MAX_ITEMS:
				return "policyTagCreateMaxItems"
	...
	def configProblem(config, leaf):
		# D30 6 and the class split: a value write belongs to tag_write (CONTROL), a
		# child is its own target, created as its own item, and a name is the target
		# path's, where tag_rename owns the new one.
	...
	def postFailureExistence(index):
		...
		# the mutation can only have been refused, never applied, so a target that is there
		...
		# existence check for this item's own target decides it. Only a plain boolean true is
		...
		try:
			...
			logger.warn("correlationId=" + correlationId + " target=" + unicode(index) + " post-failure existence check failed: " + text(exc))
			return False
		return existenceProblem(present) is None

	def collisionOutcome(index, nativeQuality):
		# D30 7: a Native outcome that is not Good, on a target Preflight found free and that
		# is now taken, is this Tool's collision case - conflict / target - not a failed
	...
	try:
		if not isinstance(items, (list, tuple, List)) or len(items) == 0:
			return toolError("invalid_argument", "items must be a non-empty array of {path, config} items.", {"reason": "itemsNotAnArray"})
		...
		for index in range(len(items)):
			...
			if "path" not in keys or "config" not in keys or len(keys) != 2:
				inputProblems.append({"index": index, "path": boundedText(pathValue, 256), "reason": "itemKeysMustBePathAndConfig"})
				continue
		...
		if inputProblems:
			return toolError("invalid_argument", "Every item must carry an absolute provider-qualified config path and a non-empty configuration object; no item was executed.", {"reason": "preflightInputFailed", "items": inputProblems})
		...
		# D30 2 and D11: tag_create takes no Precondition token, because the target must
		# not exist and so there is no earlier read of it to compare against. The
		# collision rule is the concurrency rule instead: an existing target is the
		# conflict the caller is told about, and it is what stops this Tool overwriting.
		# Every item is checked before any item executes.
		...
		for index in range(len(paths)):
			...
			if existence is None:
				preconditionProblems.append({"index": index, "path": path, "reason": "targetExists", "code": "conflict"})
				continue
		...
		if preconditionProblems:
			...
			if auditMode == "required" and not decisionRecorded:
				return toolError("operation_disabled", "The Runtime audit mode is required but the denied-mutation record could not be written; no item was executed.", {"reason": "auditAttemptFailed", "phase": "decision"})
			return toolError(code, "Every target must be absent: this Tool never overwrites an existing Tag, so an existing target is conflict and an unanswered existence check is upstream_error; no item was executed.", {"reason": "preflightPreconditionFailed", "allowlistKey": ALLOWLIST_KEY, "items": preconditionProblems, "auditRecorded": decisionRecorded})
		stage = "audit_attempt"
		...
		for index in range(len(paths)):
			...
			# The definition's name is the target's own leaf: the target path names the
			# node this creates, and a config that names another one is refused in
			# Preflight (tag_rename owns a new name).
			...
			if collisionOutcome(index, codes[0]):
				# The target appeared after the Preflight existence check and Abort left
				# it alone: this is the conflict D11 and D30 2 name, reported per item
				# with the provider's own outcome still attached.
				results.append({"path": paths[index], "status": "conflict", "reason": "targetExists", "nativeOutcome": quality(codes[0])})
				failed += 1
				continue
		...
		if encoded is None:
			return toolError("upstream_error", "The Tag creation completed but its structured result could not be serialized.", {"reason": "serializationFailure", "stage": stage, "requested": len(paths), "succeeded": succeeded, "failed": failed, "outcomeUnknown": outcomeUnknown, "auditRecorded": auditRecorded})
		...
		return toolError("upstream_error", "The Tag creation could not be completed.", {"reason": "handlerFailure", "stage": stage})
