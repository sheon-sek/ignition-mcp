def onToolCalled(builder, items):
	...
	logger = system.util.getLogger("IgnitionMCP.Runtime.TagMove")
	...
	HARD_MAX_ITEMS = 100
	POLICY_MAX_ITEMS_FIELD = "tagMoveMaxItems"
	PATH_MAX_BYTES = 2048
	...
	def buildDomain(observedEntries, outcomeCounts):
		domain = {
			...
			"summary": {"requested": len(sources), "succeeded": outcomeCounts[0], "failed": outcomeCounts[1], "outcomeUnknown": outcomeCounts[2], "notExecuted": outcomeCounts[3], "auditMode": auditMode, "auditRecorded": auditRecorded},
	...
	def absentObserved(path, subject):
		...
		if existence is None:
			return observedError(path, "upstream_error", "The target is still present after the move; it was not removed.")
		return observedError(path, "upstream_error", "The Observed state of " + subject + " could not be read.")
	...
	try:
		...
		def absentObservedOrPresent(path, subject, observedBytes, observedBudgetSpent):
			...
			if presence == "targetMissing":
				return (observedError(path, "upstream_error", "The target is not there after the move, so the change did not land where it was asked to."), observedBytes, observedBudgetSpent)
		...
		if not isinstance(items, (list, tuple, List)) or len(items) == 0:
			return toolError("invalid_argument", "items must be a non-empty array of {sourcePath, destinationPath, expectedFingerprint} items.", {"reason": "itemsNotAnArray"})
		...
		for index in range(len(items)):
			...
			if "sourcePath" not in keys or "destinationPath" not in keys or "expectedFingerprint" not in keys or len(keys) != 3:
				inputProblems.append({"index": index, "path": boundedText(sourceValue, 256), "reason": "itemKeysMustBeSourceDestinationAndFingerprint"})
				continue
			...
			if targetLeaf(source) != targetLeaf(destination):
				# One system.tag.move call moves a path list into one destination
				# folder, under each source's own name, so a destination whose leaf
				# differs would not land where the caller named it. A move that
				# renames is tag_rename's, whose new path is checked against its own
				# Target.
			...
			if not validFingerprint(expected):
				inputProblems.append({"index": index, "path": source, "reason": "fingerprintNotATagConfigFingerprint"})
				continue
			sources.append(source)
			destinations.append(destination)
			fingerprints.append(unicode(expected))
		if inputProblems:
			return toolError("invalid_argument", "Every item must carry an absolute provider-qualified source path, its fingerprint from tag_get_config, and a destination path with the same leaf name; no item was executed.", {"reason": "preflightInputFailed", "items": inputProblems})
		...
		if len(sources) > effectiveMaxItems:
			return toolError("limit_exceeded", "items exceeds the deployment's limit of " + unicode(effectiveMaxItems) + " targets; split the batch or raise " + POLICY_MAX_ITEMS_FIELD + " in the Runtime Target Policy.", {"reason": "itemsOverPolicyLimit", "requested": len(sources), "limit": effectiveMaxItems})
		...
		# D30 6: a move checks the source and the destination, so both ends are
		# measured against the Target allowlist and D30 6's _types_ rule. Owner
		# ruling 1 puts the reserved-provider rule ahead of both.
		policyProblems = []
		for index in range(len(sources)):
			...
			if reservedSource or reservedDestination:
				...
				continue
			if isUdtDefinitionTarget(source) and not matchesUdtAllowlist(source, entries, wildcardIncludesUdtTypes):
				policyProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": source, "reason": "udtDefinitionNotAllowlisted", "code": "permission_denied"})
				continue
			if not matchesAllowlist(source, entries):
				policyProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": source, "reason": "targetNotAllowlisted", "code": "permission_denied"})
				continue
			if isUdtDefinitionTarget(destination) and not matchesUdtAllowlist(destination, entries, wildcardIncludesUdtTypes):
		...
		if policyProblems:
			...
			decisionRecorded = auditWrite("decision", refusedText, "outcome=denied code=permission_denied refused=" + unicode(len(policyProblems)) + " requested=" + unicode(len(sources)))
			...
			return toolError("permission_denied", "Every source and every destination must be inside the Runtime Target Policy allowlist (a UDT definition under an explicit _types_ entry, or under * when allowlistsWildcardIncludeUdtTypes is true) and outside the reserved policy provider; no item was executed.", {"reason": "preflightTargetRefused", "allowlistKey": ALLOWLIST_KEY, "items": policyProblems, "auditRecorded": decisionRecorded})
		stage = "preflight_fingerprint"
		# D30 2 and 3: every item's source existence and fingerprint, and the
		# destination's absence, are checked before any item executes.
		preconditionProblems = []
		for index in range(len(sources)):
			source = sources[index]
			destination = destinations[index]
			try:
				sourceExistence = exists(source)
			except (Exception, JavaException) as exc:
				...
				continue
			if sourceExistence == "targetMissing":
				...
				continue
			if sourceExistence is not None:
				preconditionProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": source, "reason": sourceExistence, "code": "upstream_error"})
				continue
			try:
				actual, failure = configurationFingerprint(source)
			except (Exception, JavaException) as exc:
			...
			if failure is not None:
				preconditionProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": source, "reason": failure, "code": "upstream_error"})
				continue
			if actual != fingerprints[index]:
				preconditionProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": source, "reason": "fingerprintMismatch", "code": "conflict", "expectedFingerprint": fingerprints[index], "observedFingerprint": actual})
				continue
			try:
				destinationExistence = exists(destination)
			except (Exception, JavaException) as exc:
				...
				continue
			if destinationExistence is None:
				...
				continue
			if destinationExistence != "targetMissing":
				preconditionProblems.append({"index": index, "sourcePath": source, "destinationPath": destination, "path": destination, "reason": destinationExistence, "code": "upstream_error"})
		if preconditionProblems:
			...
			decisionRecorded = auditWrite("decision", refusedText, "outcome=denied code=" + code + " refused=" + unicode(len(preconditionProblems)) + " requested=" + unicode(len(sources)))
			if auditMode == "required" and not decisionRecorded:
				return toolError("operation_disabled", "The Runtime audit mode is required but the denied-mutation record could not be written; no item was executed.", {"reason": "auditAttemptFailed", "phase": "decision"})
			return toolError(code, "Every source must exist with the fingerprint of its own tag_get_config read, and every destination must be free: an occupied destination is conflict and a source that is not there is not_found; no item was executed.", {"reason": "preflightPreconditionFailed", "allowlistKey": ALLOWLIST_KEY, "items": preconditionProblems, "auditRecorded": decisionRecorded})
		# The Observed state reports two paths per item: the destination, which is
		# present, and the source, which is gone.
		observedPaths = []
		for index in range(len(sources)):
			observedPaths.append(destinations[index])
			observedPaths.append(sources[index])
		stage = "audit_attempt"
		targetText = ",".join(observedPaths)
		attemptRecorded = auditWrite("attempt", targetText, "outcome=attempt requested=" + unicode(len(sources)))
		...
		for index in range(len(sources)):
			if stopped:
				...
				continue
			try:
				# One system.tag.move call per item, with the source as a one-element
				# path list and the destination's parent folder as the native
				# destination: the function moves a path list into one destination
				# under each source's own name, and Preflight has already required the
				# destination's leaf to equal the source's, so the move lands exactly
				# on the path the caller named.
				codes = system.tag.move([sources[index]], targetBase(destinations[index]), COLLISION_POLICY)
			except (Exception, JavaException) as exc:
				# D30 4: Abort makes a destination that appeared in the race window
				# fail the call instead of replacing it. A destination the Gateway now
				# answers as present is that collision - conflict, never a success -
				# and it is attributable, so the rest of the batch still runs.
				if destinationAppeared(destinations[index]):
					results.append({"sourcePath": sources[index], "destinationPath": destinations[index], "status": "conflict", "reason": "destinationExists"})
					failed += 1
					continue
				logger.warn("correlationId=" + correlationId + " item=" + unicode(index) + " move is indeterminate: " + text(exc))
				results.append({"sourcePath": sources[index], "destinationPath": destinations[index], "status": "outcome_unknown"})
				outcomeUnknown += 1
				stopped = True
				continue
			outcome = singleOutcome(codes)
			if outcome is None:
				results.append({"sourcePath": sources[index], "destinationPath": destinations[index], "status": "outcome_unknown"})
				outcomeUnknown += 1
				continue
			if collisionOutcome(destinations[index], outcome):
				# The destination appeared after the Preflight existence check and Abort
				# left it alone: the conflict D11 and D30 2 name, reported per item with
				# the provider's own outcome still attached.
				results.append({"sourcePath": sources[index], "destinationPath": destinations[index], "status": "conflict", "reason": "destinationExists", "nativeOutcome": quality(outcome)})
				failed += 1
				continue
			results.append({"sourcePath": sources[index], "destinationPath": destinations[index], "status": "executed", "nativeOutcome": quality(outcome)})
			if qualityIsGood(outcome):
				succeeded += 1
			else:
				failed += 1
		stage = "audit_result"
		...
		for index in range(len(sources)):
			# Observed state: the destination is present and the source is gone.
			# A destination that is not there is an explicit observed error.
			entry, observedBytes, observedBudgetSpent = absentObservedOrPresent(destinations[index], "the moved destination", observedBytes, observedBudgetSpent)
			observed.append(entry)
			observed.append(absentObserved(sources[index], "the moved source"))
		stage = "serialization"
		notExecuted = 0
		...
		if encoded is None:
			return toolError("upstream_error", "The Tag move completed but its structured result could not be serialized.", {"reason": "serializationFailure", "stage": stage, "requested": len(sources), "succeeded": succeeded, "failed": failed, "outcomeUnknown": outcomeUnknown, "auditRecorded": auditRecorded})
		...
		if payloadBytes > OUTPUT_MAX_BYTES:
			...
			return toolError("limit_exceeded", "The structured result is " + unicode(payloadBytes) + " bytes, over the " + unicode(OUTPUT_MAX_BYTES) + "-byte output ceiling, even without the Observed state; move fewer targets, and re-read with tag_get_config.", {"reason": "outputOverLimit", "requestedBytes": payloadBytes, "limitBytes": OUTPUT_MAX_BYTES, "requested": len(sources), "succeeded": succeeded, "failed": failed, "outcomeUnknown": outcomeUnknown, "auditRecorded": auditRecorded})
		return {"structuredContent": domain}
	except (Exception, JavaException) as exc:
		logger.error("correlationId=" + correlationId + " stage=" + stage + " tag_move failed: " + text(exc))
		if dispatched:
			# D08: never replay an uncertain mutation; the agent re-reads instead.
			return toolError("outcome_unknown", "A Tag move may have been dispatched but its outcome could not be established.", {"reason": "dispatchOutcomeUnknown", "stage": stage})
		return toolError("upstream_error", "The Tag move could not be completed.", {"reason": "handlerFailure", "stage": stage})
