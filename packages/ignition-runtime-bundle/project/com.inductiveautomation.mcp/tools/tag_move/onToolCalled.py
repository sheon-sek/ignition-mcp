	...
	try:
		...
		for index in range(len(sources)):
			...
			if reservedSource or reservedDestination:
				...
				continue
			if isUdtDefinitionTarget(source) and not matchesUdtAllowlist(source, entries):
			...
			if not matchesAllowlist(source, entries):
				...
				continue
			if isUdtDefinitionTarget(destination) and not matchesUdtAllowlist(destination, entries):
		...
		if policyProblems:
			...
			return toolError("permission_denied", "Every source and every destination must be inside the Runtime Target Policy allowlist (a UDT definition only under an explicit _types_ entry) and outside the reserved policy provider; no item was executed.", {"reason": "preflightTargetRefused", "allowlistKey": ALLOWLIST_KEY, "items": policyProblems, "auditRecorded": decisionRecorded})
