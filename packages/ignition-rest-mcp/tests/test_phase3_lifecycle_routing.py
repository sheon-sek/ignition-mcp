...
def test_every_registered_tool_routes_through_the_single_invoke() -> None:
    ...
    # five Phase 5 Perspective reads, whose operation-record paths are pinned in the
    # test_phase4_*/test_phase5_* modules because a scope-carrying credential or a
    # staged Project export is needed to reach them.
    assert len(tools) == 29, "registered REST Tool functions drifted from the routed inventory"
    for node in tools:
        calls = {
            call.func.id
