# Phase 5 G5 evidence: 8.3.9 candidate row

This row closes G5 for the 8.3.9 tuple. It was composed by
`python -m tooling.compat g5 --close tests/compatibility/g5/close-8.3.9.json` from the
...
The row is `FAILED_NATIVE_BINDING`. It copies that binding status from the cited G4 row, because this stage deploys no Module and cannot measure the binding. Every Perspective case passed on this Gateway. No tuple is `SUPPORTED`.
