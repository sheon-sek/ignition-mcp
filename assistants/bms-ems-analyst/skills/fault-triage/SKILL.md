...
2. **Locate.** Find the Tag paths for every piece of equipment named, plus the equipment directly upstream and downstream of it. Use the `graphene-evidence` skill.
...
5. **Timeline.** Query the Historian from before the alarm to now. Check what changed in the same window with the `graphene-evidence` alarms-and-changes reference.
